...
This document provides an overview of how to integrate with and consume the GDPR API.
...
The GDPR API interacts with the IMS organization ID to validate permissions of the user submitting the request. It then forwards this information on to the ExC solutions to process.
...
Now that you have your data collection for submission, it's time to create your API integration. Any ExC API, such as Adobe's GDPR API, that accesses a service or content on behalf of an end user, authenticates using the OAuth and JSON Web Token (JWT) standards.
