# Azure Blob connector for Adobe Experience Platform (Legacy)

>   **Note**: The Azure Blob connector outlined in this document is being deprecated. For documentation on the updated Azure Blob connector, see the tutorial on [creating an Azure Blob or Amazon S3 source connector using the Flow Service API](../sources_tutorial/api/blob-s3-api-tutorial.md).

Adobe Experience Platform provides a RESTful API and user interface that allow you to ingest data from your Azure Blob data store.

This tutorial covers the following steps for setting up and configuring the Azure Blob connector using the [Partner Connectors API](../../../../../../acpdr/swagger-specs/partner-connectors-api.yaml):

* [Create a data connection](#create-a-data-connection) using your Azure Blob credentials.
...
The following steps cover how to create a data connection. In order to create a data connection, you require the following Azure Blob credential value:

* `{BLOB_CONNECTION_STRING}`: Your Azure Blob connection string

Using the [Partner Connectors API](../../../../../../acpdr/swagger-specs/partner-connectors-api.yaml), you can create a new connection entity for your Azure Blob account by providing your `{BLOB_CONNECTION_STRING}` in the payload of a POST request
...
You can create a dataset using one or more of the following configurations, depending on how you want to set up the ingestion behavior of your connection:
...
* A dataset configured for [incremental ingestion](#create-a-datset-for-incremental-ingestion)
      ...
      "datasets": [
        {
          ...
          "tags": {
            "connectors-objectName": [
              "https://{BLOB_URL_PREFIX}.blob.core.windows.net/{BLOB_CONTAINER}/testConfigs/IncrementalTest/YearlyData_Folder_Regex/"
            ],
...
A successful response returns the `name` and `id` of the newly created dataset under a `success` attribute. While you have control over the `name` of the dataset by providing it in the request payload, its `id` is a unique, read-only string that is automatically generated by the system.
...
> **Note:** When using incremental ingestion, you must clean up the data after every ingestion run.
...
The following table shows example values for the `isFolderRegex` tag, and how they behave for different Blob paths.

| Blob path | `isFolderRegex` | Behavior |
| ------- | ------------- | ----------- |
| https://{BLOB_URL}/FolderA  | false | All files inside FolderA having names with date-time regex values are picked. |
| https://{BLOB_URL}/FolderA/ | true | All folders inside FolderA having names with date-time regex values are picked. All files within the matching folders are also be picked. |
| https://{BLOB_URL}/FolderA/FolderB | true | All folders inside FolderA having names starting with "FolderB" and containing date-time regex values are picked. Since the Blob path does not end with a slash (`/`), the name of the last folder ("FolderB") is treated as a prefix for the regular expression being used.  |
...
Before your Blob connector ingests data for the first time, it is best practice to preview the data to see if it will be ingested as expected. The following sections demonstrate the different available methods for previewing data using the Partner Connectors API.
...
```http
GET /connections/{connectionId}/objects?object={BLOB_PATH}
```
* `{BLOB_PATH}`: The path to the Blob folder you want to preview.

#### Request

```SHELL
curl -X GET \
  'https://platform.adobe.io/data/foundation/connectors/connections/57eaaa10a6604462ae5588925/objects?object=https://{BLOB_URL_PREFIX}.blob.core.windows.net/{BLOB_CONTAINER}/testConfigs/cc-sample-data-transformed/output_folder/' \
  -H 'Authorization: Bearer {ACCESS_TOKEN}' \
  -H 'Content-Type: application/json' \
  -H 'x-api-key: {API_KEY}' \
  -H 'x-gw-ims-org-id: {IMS_ORG}' \
...
```http
GET /connections/{connectionId}/fields?object={BLOB_PATH}&fileType={FILE_TYPE}
```
* `{BLOB_PATH}`: The path to the Blob file you want to preview.
...
```SHELL
curl -X GET \
  'https://platform.adobe.io/data/foundation/connectors/connections/57eaaa10a6604462ae5588925/fields?object=https://{BLOB_URL_PREFIX}.blob.core.windows.net/{BLOB_CONTAINER}/testConfigs/IncrementalTest/YearlyData_Folder_Regex/&fileType=parquet' \
  -H 'Authorization: Bearer {ACCESS_TOKEN}' \
  -H 'Content-Type: application/json' \
  -H 'x-api-key: {API_KEY}' \
  -H 'x-gw-ims-org-id: {IMS_ORG}' \
...
```http
GET /connections/{connectionId}/rows?object={BLOB_PATH}&fileType={FILE_TYPE}
GET /connections/{connectionId}/rows?object={BLOB_PATH}&fileType=parquet&format={FORMAT}
```
* `{BLOB_PATH}`: The path to the Blob file you want to preview.
* `{FILE_TYPE}`: The format of the file to be previewed. Supported values are `parquet`, `json`, and `delimited`.
...
```SHELL
curl -X GET \
  'https://platform.adobe.io/data/foundation/connectors/connections/57eaaa10a6604462ae5588925/rows?object=https://{BLOB_URL_PREFIX}.blob.core.windows.net/{BLOB_CONTAINER}/testConfigs/IncrementalTest/YearlyData_Folder_Regex/&fileType=parquet&format=tabular' \
  -H 'Authorization: Bearer {ACCESS_TOKEN}' \
  -H 'Content-Type: application/json' \
  -H 'x-api-key: {API_KEY}' \
  -H 'x-gw-ims-org-id: {IMS_ORG}' \
...
You have now successfully created an Azure Blob connector and tested that it is correctly ingesting data into Platform. To create additional connections, see the [data connectors](../../technical_overview/acp_connectors_overview/acp-connectors-overview.md) documentation.
